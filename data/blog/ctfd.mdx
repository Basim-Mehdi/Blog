---
title: Hosting a CTF Using CTFd on AWS Lightsail with Cloudflare and Dockerized Challenges
date: '2025-07-23'      
...
````markdown
# Hosting a CTF Using CTFd on AWS Lightsail with Cloudflare and Dockerized Challenges
...
## Introduction
In this Blog, I’ll share how I hosted my own Capture The Flag (CTF) competition C00K3D CTF- Batch Zero using **CTFd**, deployed on **AWS Lightsail**, secured with **Cloudflare**, and powered by **Dockerized challenge containers** for Web and Pwn categories.
...
2. Click **Create Instance**.
   ...
   ![AWS Lightsail Instance Creation](images/lightsail-instance-create.png)

3. Select Availaible Region **closest to you**, I selected Mumbai 

   ![AWS Lightsail Change Region](public/static/images/ctfd/Change_zone.png)

   ![AWS Lightsail select region](public/static/images/ctfd/select_region.png)
...
4. Choose:
   ...
   ![AWS Lightsail Instance Creation](public/static/images/ctfd/select_instance.png)
   ![AWS Lightsail Instance Creation](public/static/images/ctfd/select_plan.png)

5. Name the instance and click **Create Instance**.

   ![AWS Lightsail Instance Creation](public/static/images/ctfd/select_name.png)
...
3. Add firewall rules to allow all ports as We will be using Container Plugin for CTFd
   ...
   ![Lightsail Networking Settings](public/static/images/ctfd/select_rules.png)

   - Select All TCP and All UDP --> Create

   ![Lightsail Networking Settings](public/static/images/ctfd/All_TCP.png)

   - Your Rules config should look like this:

   ![Lightsail Networking Settings](public/static/images/ctfd/complete_rules.png)
...
1. Buy a domain from a registrar such as [Hostinger](https://www.hostinger.com/).
   ...
   ![Get a Domain](public/static/images/ctfd/get_domain.png) 

      - Enter your **desired domain** ( without .com/pk/etc ) and select from the options given to you.

   ![Select Desired Domain](public/static/images/ctfd/domain_selection.png)
   ...
   ![Confimation Mail](public/static/images/ctfd/domain_finalized.png)

2. Add your domain to **Cloudflare**.
   - Sign up to Cloudfare and map Connect your domain

   ![Connecting domain to Cloudfare](public/static/images/ctfd/onboard_domain.png)

   - Leave the default options selected (keep the default boxes checked).

   ![Map Domain to Cloudfare](public/static/images/ctfd/map_domain_cloudfare.png)
   ...
   ![Cloudfare Nameserver](public/static/images/ctfd/cloudfare_nameserver.png)
      
      - Copy these nameservers and paste them in hostinger DNS nameserver
   
   ![Hostinger Nameserver](public/static/images/ctfd/hostinger_dnsserver.png)
...
3. Create an **A record** pointing to your Lightsail static IP.
   ...
   ![A Records Config](public/static/images/ctfd/domain_Arecords.png)
   ...
   ![Cname Configure](public/static/images/ctfd/Cname_Cloudfare.png)

5. In SSL/TLS settings:
   - Mode: **Full**
   - Enable **Always Use HTTPS**
6. Enable **Firewall Rules** to filter unwanted traffic.

![Cloudflare DNS Settings](public/static/images/ctfd/cloudflare-dns.png)
...
![SSH Login to Lightsail](public/static/images/ctfd/ssh-login.png)
...
![Docker Version Check](public/static/images/ctfd/docker-version.png)
...
![CTFd Docker Configuration](public/static/images/ctfd/ctfd-config.png)
...
![CTFd Running in Docker](public/static/images/ctfd/ctfd-running.png)
...
![Dockerized Challenges Running](public/static/images/ctfd/docker-challenges.png)

---

## Step 9: Security

* Restricted challenge ports to **Cloudflare’s IP ranges** only.
* Disabled root login via SSH.
* Kept only necessary services running.
* Used Cloudflare **WAF rules** to block suspicious traffic.

![Cloudflare WAF Rules](public/static/images/ctfd/cloudflare-waf.png)
...
![CTFd Admin Challenge Panel](public/static/images/ctfd/ctfd-admin-challenges.png)
...
![CTFd Scoreboard](public/static/images/ctfd/ctfd-scoreboard.png)

---

## Step 12: After the Event

* Exported results from **CTFd Admin → Exports**.
* Shared challenge write-ups with players.
* Stopped or deleted unused Lightsail resources to avoid costs.

![CTFd Export Results](public/static/images/ctfd/ctfd-export.png)

---

## Lessons Learned

* Lightsail was easier to set up than EC2 for small–medium events.
